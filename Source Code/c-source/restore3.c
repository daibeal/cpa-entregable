...
void process( int w,int h,Byte a[], int bw,int bh ) {
  ...
  for ( y = bh ; y < h ; y += bh ) {
    ...
    int distanciamax;
    int distanciamin;
    #pragma omp parallel private (numitera, identificadorh, distanciamax,distanciamin)
    {
      numitera=0;
      identificadorh=omp_get_thread_num();
      distanciamax=0;
     ...
     #pragma omp critical    
      ...
      if (d > distanciamax) {distanciamax=d;} 
    }
    if(y==bh) {
        printf("Hilo %d: %d iteraciones con distancias entre %d y %d.\n  ",identificadorh, numitera, distanciamin, distanciamax);
    } 
